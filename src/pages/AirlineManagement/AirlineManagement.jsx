...
            setAirlines(data);
        } catch (err) {
            setListError('Greška pri dohvaćanju aviokompanija.');
        } finally {
            setLoading(false);
        }
    };
...
                },
                body: JSON.stringify(editingAirline ? { ...formData, id: editingAirline.id } : formData),
            });

            if (!res.ok) {
                const message = await res.text();
                throw new Error(message || 'Greška pri spremanju.');
            }
...
        if (!window.confirm('Da li ste sigurni da želite obrisati ovu aviokompaniju?')) return;
...
                throw new Error(msg || 'Greška pri brisanju.');
            }
...
                                        <button className="btn" onClick={handleCloseForm}>Cancel</button>
